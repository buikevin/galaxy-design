            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between gap-2 mb-1">
                  <span
                    :class="
                      cn('font-medium truncate', !email.read && 'font-bold')
                    "
                  >
                    {{ email.from.name }}
                  </span>
                ...
                <div
                  :class="
                    cn('text-sm truncate mb-1', !email.read && 'font-semibold')
                  "
                >
                  {{ email.subject }}
                </div>
        ...
        <div class="p-4 border-b space-y-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <Button
                variant="ghost"
                size="icon"
                @click="handleEmailAction(selectedEmail.id, 'star')"
              >
                <svg
                  class="h-4 w-4"
                  :class="selectedEmail.starred && 'fill-yellow-400'"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <Button
                variant="ghost"
                size="icon"
                @click="handleEmailAction(selectedEmail.id, 'archive')"
              >
                <svg
                  class="h-4 w-4"
                  fill="none"
              ...
              <Button
                variant="ghost"
                size="icon"
                @click="handleEmailAction(selectedEmail.id, 'delete')"
              >
                <svg
                  class="h-4 w-4"
                  fill="none"
          ...
          <div class="flex items-center gap-3">
            <Avatar class="h-10 w-10">
              <AvatarImage
                v-if="selectedEmail.from.avatar"
